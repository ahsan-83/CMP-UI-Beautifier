...
const StatusBadge = ({ status }: { status: string }) => {
  ...
  return (
    <span className={cn("inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold shadow-sm border border-black/5", config.bg, config.text)}>
    ...
    <div className="bg-white rounded-2xl border border-border/60 shadow-sm overflow-hidden flex flex-col">
      ...
      <div className="p-6 border-b border-border/50 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-slate-50/50">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">Manage and track application statuses</p>
        </div>
          ...
          <tbody className="divide-y divide-border/50">
            {isLoading ? (
              ...
              filteredServices?.map((service) => (
                ...
                >
                  <td className="px-6 py-4 font-mono text-xs font-semibold text-primary">{service.id}</td>
                  ...
                  <td className="px-6 py-4 text-right">
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent align="end" className="w-48 p-2 rounded-xl shadow-xl border-border/50">
                        <DropdownMenuLabel className="text-xs font-bold text-muted-foreground uppercase tracking-wider">Change Status</DropdownMenuLabel>
                        <DropdownMenuSeparator />
                        <DropdownMenuItem onClick={() => handleStatusChange(service.id, "Active")} className="cursor-pointer font-medium text-blue-600 focus:bg-blue-50 focus:text-blue-700">
                          <Activity className="w-4 h-4 mr-2" /> Mark Active
                        </DropdownMenuItem>
                        <DropdownMenuItem onClick={() => handleStatusChange(service.id, "Completed")} className="cursor-pointer font-medium text-emerald-600 focus:bg-emerald-50 focus:text-emerald-700">
                          <CheckCircle className="w-4 h-4 mr-2" /> Mark Completed
                        </DropdownMenuItem>
                        <DropdownMenuItem onClick={() => handleStatusChange(service.id, "Rejected")} className="cursor-pointer font-medium text-rose-600 focus:bg-rose-50 focus:text-rose-700">
      ...
      <div className="p-4 border-t border-border/50 bg-slate-50 flex items-center justify-between text-sm text-muted-foreground">
        <p>Showing <span className="font-medium text-foreground">{filteredServices?.length || 0}</span> results</p>
        <div className="flex gap-2">
          <button className="px-3 py-1.5 border border-border rounded-lg bg-white hover:bg-slate-50 hover:text-foreground transition-colors disabled:opacity-50">Previous</button>
          <button className="px-3 py-1.5 border border-border rounded-lg bg-white hover:bg-slate-50 hover:text-foreground transition-colors disabled:opacity-50">Next</button>
        </div>
      </div>
    </div>
  );
