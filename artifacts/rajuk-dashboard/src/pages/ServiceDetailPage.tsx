...
import { ChevronRight, Cloud, Eye, Pencil, Server, Mail, MoreHorizontal, ChevronLeft } from "lucide-react";
    ...
    <AppLayout withSidebar>
      <div className="flex flex-col gap-6 max-w-[1400px] mx-auto">
        ...
        <div className="flex items-center text-sm font-medium text-muted-foreground gap-2">
          <Link href="/contracts" className="hover:text-primary transition-colors">Contracts</Link>
        ...
        <div className="bg-white rounded-2xl border border-border shadow-sm p-6 relative overflow-hidden">
          ...
          <div className="absolute top-6 right-6">
            <Badge variant="outline" className="font-mono text-xs border-primary/20 bg-primary/5 text-primary px-3 py-1">NDC-00075</Badge>
          </div>
          ...
          <div className="flex flex-col gap-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                ...
                <p className="text-sm text-slate-600 mt-1">Rajdhani Unnayan Kartipakkha (RAJUK), Ministry of Housing and Public Works</p>
              </div>
        ...
        <Collapsible open={packagesOpen} onOpenChange={setPackagesOpen} className="bg-white rounded-2xl border border-border shadow-sm overflow-hidden">
          <CollapsibleTrigger className="w-full p-5 flex items-center justify-between bg-slate-50/50 hover:bg-slate-50 transition-colors border-b">
            <div className="text-left">
              ...
              <p className="text-sm text-muted-foreground mt-0.5">Manage and view your assigned cloud resource packages</p>
            </div>
            <div className="flex items-center gap-4">
              <span className="text-sm font-bold text-primary bg-primary/10 px-3 py-1 rounded-full">Your Packages (189)</span>
              <ChevronRight className={`w-5 h-5 text-muted-foreground transition-transform duration-300 ${packagesOpen ? 'rotate-90' : ''}`} />
            </div>
          </CollapsibleTrigger>
          ...
          <CollapsibleContent className="p-0 animate-in slide-in-from-top-2 fade-in-20 duration-300">
                    ...
                    <tr key={pkg.id} className="hover:bg-slate-50/50 transition-colors group">
                      <td className="px-5 py-4">
                        <div className="flex items-start gap-3">
                          <div className={`p-2 rounded-lg mt-0.5 shrink-0 ${pkg.type === 'VPS' ? 'bg-blue-50 text-blue-600' : 'bg-orange-50 text-orange-600'}`}>
                            {pkg.type === 'VPS' ? <Server className="w-4 h-4" /> : <Mail className="w-4 h-4" />}
                          </div>
                          <div>
                            <p className="font-semibold text-foreground">{pkg.type === 'VPS' ? 'Virtual Private Server (VPS)' : 'Email Service'}</p>
                      ...
                      <td className="px-5 py-4 align-top pt-5">
                        {pkg.feature !== "N/A" ? (
                          <Badge variant="secondary" className="bg-primary/5 text-primary border-primary/20 font-medium whitespace-nowrap">
                            {pkg.feature}
                          </Badge>
                        ) : (
                          <span className="text-xs text-muted-foreground bg-slate-100 px-2 py-1 rounded">N/A</span>
                        )}
                      </td>
                      <td className="px-5 py-4 align-top pt-5">
                        <Badge variant="outline" className="font-mono text-xs mb-2 bg-slate-50">{pkg.detail}</Badge>
                        {pkg.extras !== "N/A" && (
                          <p className="text-xs text-muted-foreground mt-1 max-w-[180px] truncate" title={pkg.extras}>{pkg.extras}</p>
                        )}
                      </td>
                      <td className="px-5 py-4 align-top pt-5">
                        <span className="font-bold text-emerald-600 whitespace-nowrap">BDT {pkg.fee}</span>
                      </td>
            ...
            <div className="p-4 border-t border-border flex items-center justify-between bg-slate-50 text-sm">
              ...
              <div className="flex items-center gap-1">
                <Button variant="outline" size="sm" className="h-8" disabled><ChevronLeft className="w-4 h-4 mr-1" /> Previous</Button>
                <div className="flex items-center gap-1 px-2 hidden md:flex">
                  <Button variant="outline" size="sm" className="h-8 w-8 p-0 bg-primary text-primary-foreground border-primary hover:bg-primary/90 hover:text-primary-foreground">1</Button>
                  <Button variant="outline" size="sm" className="h-8 w-8 p-0">2</Button>
                  <Button variant="outline" size="sm" className="h-8 w-8 p-0">3</Button>
                  <span className="px-2 text-muted-foreground">...</span>
                  <Button variant="outline" size="sm" className="h-8 w-8 p-0">19</Button>
                </div>
                <Button variant="outline" size="sm" className="h-8">Next <ChevronRight className="w-4 h-4 ml-1" /></Button>
              </div>
            </div>
          </CollapsibleContent>
        </Collapsible>
        ...
        <Collapsible open={usersOpen} onOpenChange={setUsersOpen} className="bg-white rounded-2xl border border-border shadow-sm overflow-hidden mb-8">
          <CollapsibleTrigger className="w-full p-5 flex items-center justify-between bg-slate-50/50 hover:bg-slate-50 transition-colors border-b">
            <div className="text-left">
              <h2 className="text-lg font-bold text-foreground">Service Users</h2>
              <p className="text-sm text-muted-foreground mt-0.5">Manage billing and technical contacts for this service</p>
            </div>
            <ChevronRight className={`w-5 h-5 text-muted-foreground transition-transform duration-300 ${usersOpen ? 'rotate-90' : ''}`} />
          </CollapsibleTrigger>
          ...
          <CollapsibleContent className="p-6 animate-in slide-in-from-top-2 fade-in-20 duration-300">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              ...
              <Card className="relative group border-primary/20 bg-primary/5">
                <Button variant="ghost" size="icon" className="absolute top-2 right-2 h-8 w-8 text-primary opacity-0 group-hover:opacity-100 transition-opacity">
                  <Pencil className="w-4 h-4" />
                </Button>
                <CardContent className="p-5 pt-6">
                  <Badge variant="outline" className="mb-3 bg-white">Primary Billing</Badge>
                  <p className="font-bold text-foreground text-lg mb-1">Md. Jamal Uddin</p>
                  <p className="text-xs text-muted-foreground mb-3 font-medium">Deputy Director Finance & Accounts</p>
                  <div className="space-y-1.5 text-sm">
                    <p className="flex items-center gap-2 text-slate-600"><Mail className="w-3.5 h-3.5" /> ddflinance@rajuk.gov.bd</p>
                  </div>
                </CardContent>
              </Card>
              ...
              <Card className="relative group">
                ...
                <CardContent className="p-5 pt-6">
                  <Badge variant="secondary" className="mb-3">Secondary Billing</Badge>
                  ...
                  <div className="space-y-1.5 text-sm">
                    <p className="flex items-center gap-2 text-slate-600"><Mail className="w-3.5 h-3.5" /> drfin@rajuk.gov.bd</p>
                    <p className="flex items-center gap-2 text-slate-600"><span>📱</span> +8801777775514</p>
                  </div>
                </CardContent>
              </Card>
              ...
              <Card className="relative group border-primary/20 bg-primary/5">
                ...
                <CardContent className="p-5 pt-6">
                  <Badge variant="outline" className="mb-3 bg-white">Primary Technical</Badge>
                  <p className="font-bold text-foreground text-lg mb-1">Kaji M. Mahabubul Hoque</p>
                  <p className="text-xs text-muted-foreground mb-3 font-medium">Senior System analyst</p>
                  <div className="space-y-1.5 text-sm">
                    <p className="flex items-center gap-2 text-slate-600"><Mail className="w-3.5 h-3.5" /> saa@rajuk.gov.bd</p>
                  </div>
                </CardContent>
              </Card>
              ...
              <Card className="relative group">
                ...
                <CardContent className="p-5 pt-6">
                  <Badge variant="secondary" className="mb-3">Secondary Technical</Badge>
                  ...
                  <div className="space-y-1.5 text-sm">
                    <p className="flex items-center gap-2 text-slate-600"><Mail className="w-3.5 h-3.5" /> programmer@rajuk.gov.bd</p>
                    <p className="flex items-center gap-2 text-slate-600"><span>📱</span> +8801554001971</p>
                  </div>
                </CardContent>
              </Card>
