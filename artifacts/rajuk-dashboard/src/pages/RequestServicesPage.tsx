...
const SUB_SERVICES: Record<string, { id: string; label: string }[]> = {
  compute: [
...
function EmailDialog({ open, onOpenChange }: { open: boolean; onOpenChange: (v: boolean) => void }) {
  ...
  const handleAdminCountChange = (val: string) => {
    setAdminCount(val);
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-xl max-h-[90vh] flex flex-col overflow-hidden p-0 gap-0">
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-5">
          <h4 className="font-bold text-foreground text-sm">Configuration Options</h4>
          ...
          {/* File upload */}
          <div className="space-y-1.5">
            <p className="text-xs text-rose-500 font-medium">* Please add Excel file of email list</p>
        ...
        <DialogFooter className="p-5 bg-white border-t shrink-0 flex-row items-center justify-between">
          ...
          <div className="flex gap-3">
            ...
            <Button onClick={handleAdd} className="bg-emerald-600 hover:bg-emerald-700 text-white font-semibold">
              Add to Wish List
            </Button>
          </div>
        </DialogFooter>
...
export default function RequestServicesPage() {
  ...
  const [activeService, setActiveService] = useState<string>("vps");
  ...
  const handleCatChange = (id: string) => {
    setActiveCat(id);
    setActiveService(SUB_SERVICES[id]?.[0]?.id ?? "");
  };

  const handleVpsAdd = (pkg: typeof VPS_PACKAGES[0]) => {
    setSelectedPkg(pkg);
  ...
  const handleVpsAddToWishlist = () => {
    ...
    toast({ title: "Added to Wishlist", description: `${selectedPkg?.name} VPS has been added to your wishlist.` });
  };

  const subServices = SUB_SERVICES[activeCat] ?? [];
  const sectionTitle = subServices.find(s => s.id === activeService)?.label ?? "";
                ...
                {VPS_PACKAGES.map(pkg => (
                  <Card key={pkg.id} className="relative overflow-hidden group border-border hover:border-primary/50 transition-all hover:shadow-md">
                    ...
                    <CardContent className="p-6 relative z-10 flex flex-col h-full">
                      <Badge variant="outline" className="w-fit mb-4 bg-white shadow-sm font-bold tracking-widest text-[10px] uppercase border-primary/20 text-primary">
                        {pkg.name}
                      </Badge>
                      <h3 className="text-xl font-bold text-foreground mb-4 pr-12 leading-tight">
                        {pkg.specs.split(", ").map((spec, i) => <span key={i} className="block">{spec}</span>)}
                      </h3>
                      <div className="mt-auto pt-6 flex items-end justify-between">
                        ...
                        <Button onClick={() => handleVpsAdd(pkg)} className="bg-primary hover:bg-primary/90 shadow-sm rounded-xl">
                    ...
                    <div className="space-y-3 divide-y divide-slate-100">
                      {EMAIL_FEATURES.map((f, i) => (
                        <div key={i} className={`flex items-start justify-between gap-4 ${i > 0 ? "pt-3" : ""}`}>
                          ...
                          {f.price && (
                            <span className="text-sm font-bold text-primary whitespace-nowrap shrink-0">{f.price}</span>
                          )}
                        </div>
                      ))}
                    </div>
        ...
        <DialogContent className="max-w-3xl max-h-[90vh] flex flex-col overflow-hidden bg-slate-50/50 p-0">
            ...
            {selectedPkg && (
              <div className="flex flex-wrap items-center gap-3 mt-4">
                <Badge className="bg-primary/10 text-primary border-primary/20 hover:bg-primary/10">
                  <div className="w-2 h-2 rounded-full bg-primary mr-2" />{selectedPkg.name}
                </Badge>
          ...
          <DialogFooter className="p-6 bg-white border-t shrink-0 flex-row items-center justify-between">
            <div className="text-xl font-bold text-primary">
              Total Cost: ৳ {selectedPkg?.price.toLocaleString() ?? 0}
            </div>
            <div className="flex gap-3">
              <Button variant="outline" onClick={() => setVpsDialogOpen(false)}>Cancel</Button>
              <Button onClick={handleVpsAddToWishlist} className="bg-emerald-600 hover:bg-emerald-700">Add to Wish List</Button>
            </div>
          </DialogFooter>
        </DialogContent>
      </Dialog>
